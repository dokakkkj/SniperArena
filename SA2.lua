...
-- ============== SISTEMA SILENT AIM COM VISIBLE CHECK (SEM PRIORIDADE) ==============
...
-- 🔥 LISTA DE TODAS AS PARTES DO CORPO (SEM PRIORIDADE)
local BodyParts = {
    "Head",
    "HumanoidRootPart",
    "UpperTorso",
...
-- 🔥 FUNÇÃO PARA VERIFICAR SE UMA PARTE ESTÁ VISÍVEL (RAYCAST)
local function IsPartVisible(Part)
    ...
    if HitParent and HitParent == Part.Parent then
        return true
    end
...
-- 🔥 FUNÇÃO PARA ENCONTRAR A PRIMEIRA PARTE VISÍVEL (SEM PRIORIDADE)
local function GetFirstVisiblePart(Character)
    if not Character then return nil, nil end
    
    -- Itera por todas as partes do corpo em ordem
    for _, partName in ipairs(BodyParts) do
        local Part = Character:FindFirstChild(partName)
        if Part and Part:IsA("BasePart") and IsPartVisible(Part) then
            -- Verifica se a parte está na tela
...
-- 🔥 FUNÇÃO PARA PEGAR POSIÇÃO - PRIMEIRA PARTE VISÍVEL
local function GetEntityPosition(EntityModel)
    local Part, Position = GetFirstVisiblePart(EntityModel)
...
-- ============== CONTROLES ==============
print("🔥 Silent Aim com VISIBLE CHECK (SEM PRIORIDADE) carregado!")
print("🎯 Mira na PRIMEIRA parte visível que encontrar!")
...
game:GetService("Players").LocalPlayer.Chatted:Connect(function(msg)
    if msg:lower():match("/silent") then
        if msg:lower():match("on") then
            ...
            print("✅ Silent Aim ATIVADO (Mirando na primeira parte visível)")
